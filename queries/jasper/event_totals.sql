...
select x.event_type as e_type, sum(x.event_count) as e_count, x.utility_id as e_utility_id from
(select all_types.t as event_type, 0 as event_count, all_types.utility_id as utility_id
    ...
    where d.id between
    cast(to_char(cast($P{start_date} as timestamp), 'YYYYMMDD') as int)
    and
    cast(to_char(cast($P{end_date} as timestamp), 'YYYYMMDD') as int)
    union
